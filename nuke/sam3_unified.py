...
def create():
    ...
    try:
        ...
        group['maskSource'].setTooltip('Masks stay in RAM. Analyze reuses matching preview frames; expired frames are recomputed. No PNGs are written.')
...
def configure_index(engine):
    ...
    index.setTooltip('0 = largest mask, 1 = second largest. Select one object; re-ranked per frame.')
...
def ensure_solve_controls(group):
    ...
    knobs['analyzeRange'].setTooltip('Generate and store the frame range in RAM. Use Solve beside Motion to calculate tracking curves.')
